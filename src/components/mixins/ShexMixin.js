...
export default {
    ...
    async load_remote_schema(shape_url){
      ...
      window.shexLoader.load([shape_url], [], [], []).then(loaded => {
        if (loaded.schema){
          ...
          this.$store.commit('local/setSchema',loaded.schema)
          //app.schema = JSON.stringify(loaded.schema);
  ...
  async splitSchema(schema){
    ...
    this.$store.commit('local/setCurrentShape', this.shapes[0].id)
  ...
  this.$store.commit('local/setShapes', this.shapes)
  this.$store.commit('local/setFShapes', this.footprint_shapes)
  this.$store.commit('local/setTShapes', this.translate_shapes)
}
}
}
